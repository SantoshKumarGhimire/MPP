...
public class Main {
	...
	private static void login(List<Customer> list) {
		...
		StorageSingelton.INSTANCE.add(StorageKey.LOGGED_IN.toString(), storageList);
		for(Customer c : list) {
	...
	private static void showLoggedIn() {
		List<String> stored = (List<String>)StorageSingelton.INSTANCE.get(StorageKey.LOGGED_IN.toString());
